...
class TensorforceAgent(Agent):
    ...
    def train(self, env, nb_steps):
        ...
        while step_count < nb_steps:
            ...
            while not done:
                t0 = time.time()
                if episode_step_count % 3 == 0:
                    t1 = time.time()
                    action = self.agent.act(obs)
                    print('action generation', time.time() - t1)
                    stored_action = action
                else:
                    action =  stored_action
                t3 = time.time()
                obs, rew, done, info = env.step(action)
                print('step update', time.time() - t3)
                total_rew += rew
                if episode_step_count % 3 == 0:
                    t2 = time.time()
                    self.agent.observe(reward=rew, terminal=done)
                    print('action observation', time.time() - t2)
                episode_step_count += 1
                print('total step time', time.time() - t0)
